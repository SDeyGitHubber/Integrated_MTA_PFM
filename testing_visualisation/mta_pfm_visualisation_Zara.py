...
from models.mta_pfm_model_Zara import IntegratedMTAPFMModel_Zara  # <- adjust path if needed

# Zara dataset loader with int(frame/10)
...
# Visualization function (unchanged in structure)
def plot_agents_individually_with_neighbors(dataset, device, model, sample_idx=0,
    ...
    for ego_idx in range(num_agents):
        ...
        for n_idx in range(neighbor_positions.shape[0]):
            ...
            neighbor_count += 1
        plt.title(f"Agent {ego_idx} and Neighbors (MTA+PFM Predictions)")
        plt.xlabel('X')
        plt.ylabel('Y')
...
if __name__ == "__main__":
    zara_csv_path = '/home/dasari-raj-vamsi/Desktop/iDataHub_running/data/crowds_zara02_test_cleaned.txt'
    checkpoint_path = '/home/dasari-raj-vamsi/Desktop/iDataHub_running/checkpoints/mta_pfm_trajectory_model_Zara_5eps.pth'
    dataset = PFM_TrajectoryDataset_Zara(zara_csv_path, history_len=8, prediction_len=12)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = IntegratedMTAPFMModel_Zara().to(device)
